...
export function CakePayment({quote,cake,pickup}:{quote:CakeQuote;cake:CakeConfig;pickup:string}) {
  ...
  useEffect(() => () => { active.current=false; request.current?.abort(); }, []);
  ...
  return <form className={styles.form} onSubmit={async event=>{
    ...
    try {
      if (!attempt.current) {
        const sessionResponse = await fetch(apiUrl("/api/cake/checkout-session"), { method: "POST", signal: request.current.signal });
    ...
    } finally {request.current=null;lock.current=false;if(active.current)setBusy(false);}
  }}>
    <div>
